...
<script src="http://d3js.org/d3.v3.min.js"></script>
 ...
 <script>
    ...
    function Ball(svg, x, y, id, color, aoa, weight) {
        ...
        this.Move = function () {
            ...
            if (parseInt(svg.attr('width')) < (thisobj.posX + thisobj.radius)) {
                thisobj.posX = parseInt(svg.attr('width')) - thisobj.radius;
                thisobj.aoa = Math.PI - thisobj.aoa;
                thisobj.vx = -thisobj.vx;
            }

            // stops balls disapearing behind border
            ...
            if (parseInt(svg.attr('height')) < (thisobj.posY + thisobj.radius)) {
                thisobj.posY = parseInt(svg.attr('height')) - thisobj.radius;
    ...
    function Initialize(containerId) {
        ...
        width = document.getElementById('drawArea').offsetWidth;
        height *= 0.8;
    ...
    function OnSpeedChange(val) {
      var vxBar = [];
      var vyBar = [];
      speedLim = speedLimFactor * ballLimit;
        ...
        if (val == "+"){
          if (balls.length < ballLimit){
            ...
            for (var i = balls.length; i < balls.length + extraBalls; ++i) {
              newBalls.push(new Ball(svg, width/2, height/2, 'n'+(i+1).toString(), "blue", Math.PI / (i+1), (i%2)==0?5 : (5+i)));
            }
...
<div id="mainDiv">
    <div id="drawAreaOuter" style="width:100%; height:100%;  ">
      <div id="drawArea" style="width:100%; height:100%; border:1px solid gray;"></div>
        <div id="menuTop" > 
            Temperature
              <div>
